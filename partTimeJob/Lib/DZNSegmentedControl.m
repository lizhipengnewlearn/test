...
- (void)initialize
{
    ...
    _height = 47;
    _selectionIndicatorHeight = 1.0;
    _animationDuration = 0.5;
    _showsCount = NO;
    _autoAdjustSelectionIndicatorWidth = YES;
    
    _selectionIndicator = [UIView new];
    _selectionIndicator.backgroundColor = THEMEBLUECOLOR;
    [self addSubview:_selectionIndicator];
    
    _hairline = [UIView new];
    _hairline.backgroundColor =LINECOLOR;
...
- (void)layoutSubviews
{
    ...
    //居中显示button
        ...
        [button setTitleEdgeInsets:UIEdgeInsetsMake(topInset, 0, topInset, 0)];
...
- (CGRect)selectionIndicatorRect
{
    ...
    frame.origin.y = (_barPosition > UIBarPositionBottom) ? 0.0 : (button.frame.size.height-self.selectionIndicatorHeight-6);
    ...
    if (self.autoAdjustSelectionIndicatorWidth) {
        ...
        frame.size = CGSizeMake(10, self.selectionIndicatorHeight);
...
- (void)setTintColor:(UIColor *)color
{
    ...
    [_selectionIndicator setBackgroundColor:THEMEBLUECOLOR];
}
        ...
        if (self.showsCount) {
            ...
            if (state == UIControlStateNormal) {
                ...
                if (state == UIControlStateSelected) {
                    self.selectionIndicator.backgroundColor = THEMEBLUECOLOR;
                }
            }
        } else {
            [attributedString addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:13] range:NSMakeRange(0, attributedString.string.length)];
